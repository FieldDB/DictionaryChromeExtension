...
var IMAGE_LINK_TEMPLATE = '%protocol%//%languagewikicode%.wiktionary.org/wiki/%file%';
...
var IMAGE_ICON_URL = chrome.runtime.getURL('img/ic_action_camera.png');
var SPEAKER_ICON_URL = chrome.runtime.getURL('img/ic_action_headphones.png');
...
// Main initialization function. Loads options and sets listeners.
  ...
  function createPopup(query, x, y, windowX, windowY, fixed) {
    ...
    if (frame_ref) {
      ...
      setTimeout(function() {
        createPopup(query, x, y, windowX, windowY, fixed);
      }, 50);
      return;
    }
